import {jobAge,compareNewest,ageBreakdown} from './age.mjs';
...
function drawPins(){if(!map)return;layer.clearLayers();markers.clear();const groups=new Map();for(const j of filtered){for(const geo of allLocations(j)){if(!Number.isFinite(geo.lat)||!Number.isFinite(geo.lng))continue;const key=`${geo.lat},${geo.lng}`;if(!groups.has(key))groups.set(key,[]);if(!groups.get(key).some(x=>x.id===j.id))groups.get(key).push({...j,geo});}}
 ...
 const newest=[...jobs].sort(compareNewest)[0],age=jobAge(newest.postedAt),grouped=jobs.length>1,size=grouped?32:16;
 const location=newest.geo.city||newest.location,title=grouped?`${location}: ${jobs.length} jobs at this location. Open for posting ages.`:`${location}: 1 job; ${age.label}`;
 const marker=L.marker(mapPosition(newest.geo),{icon:L.divIcon({className:`pin${grouped?' pin-group':''}`,html:`<span style="background:${grouped?'#233d4d':age.color};width:100%;height:100%;border-radius:50%;display:grid;place-items:center">${grouped?jobs.length:''}</span>`,iconSize:[size,size],iconAnchor:[size/2,size/2]}),title,alt:title,keyboard:true}).addTo(layer);
